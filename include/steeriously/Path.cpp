...
steer::Path::Path(int NumWaypoints, std::vector<Vector2>& waypoints)
: m_looped(false)
, m_numWaypoints(0)
...
std::vector<Vector2> steer::Path::createRandomPath(int NumWaypoints, float MinX, float MinY, float MaxX, float MaxY)
{
    m_wayPoints.clear();
