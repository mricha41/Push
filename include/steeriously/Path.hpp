    ...
    class Path
    {
        public:
            ...
            **/
            ...
            /**
            * \fn Path(int NumWaypoints, std::vector<steer::Vector2>& waypoints)
            * \brief Constructor used for creating a path from a list of waypoints.
            * \param NumWaypoints - a plain old int.
            * \param waypoints - a std::vector of steer::Vector2.
            **/
            Path(int NumWaypoints, std::vector<steer::Vector2>& waypoints);
            ...
            **/
            ...
            /**
            * \fn std::vector<steer::Vector2> createRandomPath(int NumWaypoints, float MinX, float MinY, float MaxX, float MaxY)
            ...
            * \param MaxY - a plain old float.
            **/
            std::vector<steer::Vector2> createRandomPath(int NumWaypoints, float MinX, float MinY, float MaxX, float MaxY);
            ...
            **/
            ...
            /**
            * \fn std::vector<steer::Vector2> getPath() const;
            * \brief Returns the std::vector of waypoints in the path.
            **/
            std::vector<steer::Vector2> getPath() const {return m_wayPoints;}

        private:

            std::vector<steer::Vector2>               m_wayPoints;///< Waypoints used to define the path the agent will be steered along.

            //points to the current waypoint
            std::vector<steer::Vector2>::iterator     m_currentWaypoint;///< An iterator (pointer) that points to the current waypoint.
